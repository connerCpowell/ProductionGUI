...
from settings import MOVE_COMPLETED, SETTLE, VELOCITY_MODE_ACTIVE, POSITION_MODE_ACTIVE, TORQUE_MODE_ACTIVE
...
class E20:
    # define cover_open() ahead of where I need to use it if ret == LID_OPEN:
#     print("Lid open detected, please close the lid")
#     sys.exit(LID_OPEN)
# if ret == LID_OPEN_TEST:
#     print("Lid opened during test. Test failed")
#     sys.exit(LID_OPEN_TEST)as a callback in __init__
    def cover_open(self):
    ...
    def run_assay(self, filename=None, version='01'):
        ...
        assay_id = filename.split('/')[len(filename.split('/'))-1]
        model = assay_id.split('-')[1]
        # check first two characters of the model and figure out is we're clear or black
        if 'CD' in model[:2].upper():
            self.cassette_colour = 'clear'
        elif 'BD' in model[:2].upper():
            self.cassette_colour = 'black'
        msg = 'Using a %s plastic disk' % self.cassette_colour
        logging.info(msg)
        print(msg)
