...
class Window(Frame):
    ...
    def init_window(self):
        ...
        def wobble():
            wob = int(varWob.get())
            shak = int(varShake.get())
            t = int(varTime.get())
            print("w, s, t", wob, shak, t)
            self.e20.motor.wobble(shake=shak, duration=t, swingX=wob)
        ...
        def ImageArray():
            ...
            self.e20.camera.capture(
                            flash=True,
                            filename='/home/autolab/Desktop/images/'+varNombre.get()+'_igg.tif'
                            )
            sleep(2)
            self.e20.motor.position_abs(0)
            ...
            self.e20.camera.capture(
                            flash=True,
                            filename='/home/autolab/Desktop/images/'+varNombre.get()+'_igm.tif'
                            )
        ...
        def spin_osc():
            ...
            while repeat > 0:
                ...
                for (_, speed, duration) in speeds:  # _ means we don't care about that value
                    ...
                    self.e20.motor.spin(rpm=speed, spin_time=duration)
                repeat -= 1  # keep decreasing by 1
            ...
            if loops != 0:  # beware of special case where loops=0 is passed in
                ...
                for (idx, values) in enumerate(SPIN_TASKS, start = 1):
                    ...
                    self.e20.motor.spin(rpm=values[0],
                                    spin_time=values[1],
                                    stopping=values[2])
                print('finished')
        ...
        self.varemail = Label(root, text = 'shake', fg = 'Black', font = Labels )
        self.varemail.place(x = 10, y = 75)
        varShake = Entry(root)
        varShake.insert(END,'300')
        varShake.place(x = 120, y = 75)
        ...
        self.varRecip.place(x = 10, y = 100)
        varRecip = Entry(root)
        varRecip.insert(END,'3')
        varRecip.place(x = 120, y = 100)
        ...
        assayButton = Button(self, text = "Wobble", font = Buttons, fg='Green', command = wobble)
        assayButton.place(x = 300, y= 225)
        ...
        # Fans
        FansButton = Button(self, text = "spin_Osc", font = Buttons, fg='Red', command = spin_osc )
